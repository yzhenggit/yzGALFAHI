...
def filter_cube(data, fregion, siglevel=2):
    ...
    return:
    new_data: data with values lower than med+siglevel*sigma will be turned into 0
    cube_mask: same shape as data or new_data, with True = pixels with noises

    History:
    updated as of 2016.10.03. Yong Zheng @ Columbia Astro
    07/23/21, add sigma clipping method to filter noise, yz.
    '''
    aa = fregion
    ...
    cut_thres = med + siglevel*sigma

    sig3d = np.reshape(cut_thres, (data.shape[0], 1, 1))
    new_data = data.copy()

    cube_mask = (new_data <= sig3d) # True = noise
    new_data[cube_mask] = 0.

    return new_data, cube_mask
